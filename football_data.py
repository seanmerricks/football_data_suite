...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Hr(style={'margin':'20px 0px', 'padding':0, 'border':'0', 'backgroundColor':colours['line_colour'], 'height':'1px'}),
            # team stats over years controls div
            html.H2('Team Year On Year', style={'font-size':'1.4em'}),
            html.Div([
                html.H2('Choose team.', style={'font-size':'1.2em'}),
                dcc.Dropdown(id='team-picker1', options=teams, value=''), #value=teams[0]['value']
            ]),
            html.Div([
                html.H2('Choose season.', style={'font-size':'1.2em'}),
                dcc.Checklist(id='season-picker1', options=seasons, values='', labelStyle={'margin':'10px 0px', 'display':'block'}), #seasons[0]['value']
            ]),
            html.Hr(style={'margin':'20px 0px', 'padding':0, 'border':'0', 'backgroundColor':colours['line_colour'], 'height':'1px'}),
        ...
        html.Div([
            ...
            html.Div([
                dcc.Graph(id='graph'),
            ], style={'width':'60%', 'float':'left', 'display':'inline-block', 'borderRight':'1px solid #E1E1E1', 'marginBottom':'20px', 'paddingRight':'20px'}),
            html.Div([
                dcc.Graph(id='table'),
...
def update_team_graph(chosen_team, chosen_season):
    ...
    for season in chosen_season:
        ...
        traces.append(go.Bar(x = ['Points', 'Goals For', 'Goals Against', 'Yellow Cards', 'Red Cards'],
                             y = [sum(points_home + points_away), goals_for_home + goals_for_away, goals_against_home + goals_against_away, yellow_cards_home + yellow_cards_away, red_cards_home + yellow_cards_away],
                             name = season))
